...
{
	if (ui32FPBegin >= ui32FPEnd || !fp)
		return 0;
	fseek(fp,ui32FPBegin, SEEK_SET);
	if (ui32FPEnd - ui32FPBegin < CHUNK_MIN_SIZE)
		return 0;
	...
	while(ui32FP < ui32FPEnd)
	{
		...
		if (pParent && !pParent->m_pChild)
			pParent->m_pChild = pChunk;

		if (pBrother)
			pBrother->m_pSibling = pChunk;
...
DataChunk::stChunk *DataChunk::SerializeIn_Helper(MemoryStream* data,uint ui32FPBegin,uint ui32FPEnd,stChunk *pParent,stChunk *pBrother)
{
...
void DataChunk::SerializeIn(MemoryStream* data, uint size)
{
	if(!data)return;
...
void DataChunk::SerializeOut_Helper(MemoryStream *pStream, stChunk *p)
{
	if(!p || !pStream)return;
...
void DataChunk::save(MemoryStream* pStream)
{
	if(!m_pChunk)return;
	if(!pStream)return;
...
void DataChunk::operateChunk(uint ui32ID,MemoryStream* pData,uint size)
{
	return;
}
...
DataChunk::stChunk* DataChunk::readChunk(MemoryStream* pData,bool bWowFormat)
{
	uint ui32Type;
	uint ui32ChunkSize,ui32DataSize;
	uchar *pChunkData;
...
DataChunk::stChunk* DataChunk::beginChunk(MemoryStream* pData,bool bWowFormat)
{
...
DataChunk::stChunk* DataChunk::nextChunk(MemoryStream* pData,bool bWowFormat)
{
...
DataChunk::stChunk* DataChunk::beginChunk(uint ui32Type,MemoryStream** pDataStream)
{
